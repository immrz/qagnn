...
def load_sparse_adj_data_with_multi_view_contextnode(adj_pk_path, max_node_num, num_choice, num_view, args,
    ...
    for idx, _data in tqdm(enumerate(adj_concept_pairs), total=n_samples, desc='loading adj matrices'):
        ...
        concept_ids[idx, num_view:num_concept] = torch.tensor(concepts)
        # this is the "concept_id" for contextnodes
        concept_ids[idx, :num_view] = torch.tensor(range(contextnode_start, contextnode_start + num_view))
        ...
        node_type_ids[idx, 1:num_view] = 4 if args.view_special_node_type else 3  # views of contextnode
        node_type_ids[idx, num_view:num_concept][torch.tensor(qm, dtype=torch.bool)[:num_real]] = 0
        node_type_ids[idx, num_view:num_concept][torch.tensor(am, dtype=torch.bool)[:num_real]] = 1
        ...
        j, k = j + num_view, k + num_view       # increment index by num_view
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length,
                                          num_mask_view=0, mask_view_prob=0.15, view_shuffle=False):
    ...
    def generate_mask_views(s: str, n: int, p: float, tokenizer: RobertaTokenizer) -> List[List[str]]:
        ...
        views = np.stack([tokens for _ in range(n + 1)], axis=0)
    ...
    def convert_examples_to_features(examples, label_list, max_seq_length,
        ...
        for ex_index, example in enumerate(examples):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                tokens_a = context.copy()  # already tokenized
